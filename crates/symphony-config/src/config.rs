...
pub struct TrackerConfig {
  ...
  /// SPEC_ADDENDUM_1 A.2.1: label the agent adds when claiming; required; included in effective exclude so claimed issues are not re-dispatched.
  #[validate(length(min = 1, message = "tracker.claim_label required"))]
  pub claim_label: String,
  /// SPEC_ADDENDUM_1 A.3.6: label the agent may add when a PR is open; required; should be in exclude_labels if used.
  #[validate(length(min = 1, message = "tracker.pr_open_label required"))]
  pub pr_open_label: String,
  ...
  /// Base branch for worker branches and PR target (e.g. main, develop). Required.
  #[validate(length(min = 1, message = "tracker.pr_base_branch required"))]
...
impl TrackerConfig {
  ...
  pub fn effective_exclude_labels(&self) -> Option<Vec<String>> {
    ...
    if !base
      .iter()
      .any(|l| l.eq_ignore_ascii_case(&self.claim_label))
    {
      base.push(self.claim_label.clone());
    }
...
/// main_repo_path is required: worker development always uses a git worktree and per-issue branch (SPEC_ADDENDUM_1 A.3.1).
#[derive(Debug, Clone)]
pub struct WorktreeConfig {
  pub root: PathBuf,
  /// Path to the main git repo (worktree or clone). Required. Each issue gets a git worktree and branch created before work starts.
...
impl ServiceConfig {
  ...
  pub fn validate_dispatch(&self) -> Result<(), ConfigValidationError> {
    ...
    if self.worktree.main_repo_path.as_os_str().is_empty() {
      return Err(ConfigValidationError::Worktree(
        "worktree.main_repo_path is required (worker development must use a git worktree and branch)".into(),
      ));
    }
    if self.worktree.root.as_os_str().is_empty() {
      return Err(ConfigValidationError::Worktree(
...
mod tests {
  ...
  fn minimal_service_config() -> ServiceConfig {
    ServiceConfig {
      ...
      worktree: WorktreeConfig {
        ...
        main_repo_path: std::env::temp_dir().join("symphony_main"),
      },
  ...
  fn effective_exclude_labels_exclude_only() {
    ...
    assert!(eff.contains(&"b".to_string()));
    assert!(eff.contains(&"claimed".to_string()));
  }
