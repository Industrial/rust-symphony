...
- **Tests:** Use [cargo-nextest](https://nexte.st/) for faster, parallel test runs: Per-crate `cargo test` remains available via `moon run <crate>:test`. **Sandbox/E2E:** `devenv shell -- cargo test -p symphony-sandbox --features firecracker --test integration_firecracker` (skips when `SYMPHONY_SANDBOX_INTEGRATION`, `SYMPHONY_KERNEL_PATH`, `SYMPHONY_ROOTFS_PATH` are unset; see [docs/SPEC/16-testing.md](docs/SPEC/16-testing.md) §17.9).
